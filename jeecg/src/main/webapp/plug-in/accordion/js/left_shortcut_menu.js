...
function goToTabWithHref(subtitle, url, icon) {
	addTab(subtitle, url, icon);
	var selectedTab = $('#maintabs').tabs('getSelected');
	if (url.indexOf('isHref') != -1) {
		//TODO 如果还是通过iframe加载的界面，怎么调用?
	} else {
		//找到iframe后调用特定的函数
		var iframe = selectedTab.find("iframe").get(0);
		//alert(iframe.src);
		//alert(iframe.length);
		...
		//alert(win.document.getElementById("ddoMsgListtb"));
		//alert(win.document.getElementById("ddoMsgListtb"));
