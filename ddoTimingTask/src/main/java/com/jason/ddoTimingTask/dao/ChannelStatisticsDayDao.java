 ...
 *
 */
public class ChannelStatisticsDayDao extends BaseDao {

	private static final Logger logger = Logger
			.getLogger(ChannelStatisticsDayDao.class);

	private final static String GET_CHANNEL_STATISTICS_DAY = "select id from ddo_channel_statistics_day where sum_date = ? and channel_id = ?";
	private final static String INSERT_CHANNEL_STATISTICS_DAY = "insert into ddo_channel_statistics_day(id, sum_date, msisdn_num, sum_amount, msg_num, send_success_num, send_fail_num, bill_success_num, bill_fail_num, channel_id)"
			+ " values(?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
	private final static String ADD_MSG_NUM = "update ddo_channel_statistics_day set msg_num = msg_num + ? where id = ?";
	private final static String ADD_MSG_AND_MSISDN_NUM = "update ddo_channel_statistics_day set msg_num = msg_num + ?, msisdn_num = msisdn_num + ? where id = ?";
	 ...
	 * @throws DaoException
	 */
	public ChannelStatisticsDay getChannelStatisticsDay(int sumDate,
		...
		try {
			...
			PreparedStatement pstmt = conn
					.prepareStatement(GET_CHANNEL_STATISTICS_DAY);
			pstmt.setInt(1, sumDate);
			pstmt.setString(2, channelId);
			ResultSet rs = pstmt.executeQuery();
			if (rs.next()) {
				record = new ChannelStatisticsDay();
	...
	public void saveChannelStatisticsDay(ChannelStatisticsDay record)
			throws DaoException {
		Connection conn = null;
		try {
			conn = super.getConnection();
			...
			logger.error("exception when saveChannelStatisticsDay", e);
			throw new DaoException(e.getMessage());
		} finally {
			super.closeConnction(conn);
		}
	 ...
	 * @throws DaoException
	 */
	public boolean isMsisdnExist(int sumDate, long msisdn, String channelId)
			throws DaoException {
		boolean exist = false;
		Connection conn = null;
		try {
	...
	public void saveMsisdn(long msisdn, int sumDate, String channelId)
			throws DaoException {
		Connection conn = null;
		try {
			conn = super.getConnection();
